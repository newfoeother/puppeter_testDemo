...
async function run()
{
  ...
  // 2.打开登录页面
  // page.click('a[name="space"]')
  await timeout(1000);

  // 3.输入账号密码
  page.type('input[id=login-username]', '17602163200')
  await timeout(500);
  page.type('input[id=login-passwd]', 'qiong521')
  await timeout(1000);
...
async function calculateDistance()
{
  const distance = await page.evaluate(() =>
  {
    ...
    function compare(document)
    {
      ...
      // 对比像素  (对比像素  不懂)
      for (let i = 57;i < 260;i++)  // i  为什么从57  开始
      {
        for (let j = 1;j < 160;j++)
        {
      ...
      // 返回像素差最大值跟最小值，经过调试最小值往左小7像素，最大值往左54像素  (7, 7)?;
...
async function getBtnPosition()
{
  ...
  {
    const { offsetLeft, offsetTop } = document.querySelector('.geetest_wind')
    return { btn_left: offsetLeft + 88, btn_top: offsetTop + 192 }


    // return { btn_left: offset(btn, "left"), btn_top: offset(btn, "top") }
  })
...
async function tryValidation(distance)
{
  ...
  {
    // console.log('do  what')
    // if (page.url().indexOf('https://www.bilibili.com') !== -1)
    // {
    return '验证成功'
    // }
  ...
  // if (page.url().indexOf('https://www.bilibili.com') !== -1)
  // {
  //   return { isSuccess: isSuccess === '验证成功' }
  // } else
  // {
...
async function searchUp(name)
{
  console.log(page.url().indexOf('https://www.bilibili.com') !== -1)
  if (page.url().indexOf('https://www.bilibili.com') !== -1)
  {
    page.type('input[class=search-keyword]', name)
    page.click('button[class=search-submit]')
    await timeout(2000);
