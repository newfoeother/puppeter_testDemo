...
async function run()
{
    ...
    await page.goto('https://www.qdfuns.com/portal.php');
    await timeout(1000);
    ...
    // 3.输入账号密码
    page.type('input[data-type=email]', '1')
    await timeout(500);
    page.type('input[placeholder=密码]', '1')
    await timeout(1000);
...
async function getBtnPosition()
{
    const btn_position = await page.evaluate(() =>
    {
        ...
        return { btn_left: outerWidth / 2 - 104, btn_top: outerHeight / 2 + 59 }
    })
